...
@section('css')
    ...
    <link href={{ asset('css/tema/editar.css') }} rel="stylesheet">
@endsection
...
@section('main')
    <header class="md:flex md:justify-center background background-dos w-full">
        <section class="form-container mx-auto w-10/12 md:w-6/12 lg:w-5/12 background background-dos px-6">
            <form id="tema-editar" class="px-4 my-12 rounded-lg lg:px-4 rounded w-full bg-white none" action="/tema/{{$tema->id_tema}}/editar" method="post"
                data-rules="{{json_encode($validation->rules)}}"
                data-messages="{{json_encode($validation->messages)}}">
                <header>
                    <h2 class="text-2xl text-center py-8">Editar "{{$tema->nombre}}"</h2>
                </header>
                @csrf
                @method('PUT')
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="nombre"
                        ...
                        type="text" placeholder="Nombre *" title="Campo obligatorio" value="{{old('nombre', $tema->nombre)}}">
                ...
                <div class="mb-4 flex justify-center flex-wrap mb-8 outline-none">
                    <select class="form-input p-3 outline-none bg-white" name="id_organismo" title="Campo obligatorio">
                        <option {{ !old("id_organismo", $tema->id_organismo) ? 'selected' : '' }} disabled>Organismo *</option>
                        <option value="1" {{ old("id_organismo", $tema->id_organismo) == 1 ? 'selected' : '' }}>INAES</option>
                        <option value="2" {{ old("id_organismo", $tema->id_organismo) == 2 ? 'selected' : '' }}>Otros organismos</option>
                    </select>
                    ...
                    <ul class="obras my-8">
                        @foreach($obras as $obra)
                            <li class="checkbox mx-auto w-8/12 md:w-7/12 lg:w-10/12">
                                @if(is_array(old('obras', $tema->obras)) && in_array($obra->id_obra, old('obras', $tema->obras)))
                ...
                <div class="flex items-center justify-center lg:justify-center py-4">
                    <input class="form-submit tema-editar btn btn-dos p-2 px-8 my-4" type="submit" value="Editar Tema">
                </div>
            </form>
        </section>
    </header>
...
@section('js')
    ...
    <script type="module" src="{{asset('js/tema/editar.js')}}"></script>
@endsection
