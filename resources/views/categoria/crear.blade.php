...
@section('css')
    ...
    <link href={{ asset('css/categoria/crear.css') }} rel="stylesheet">
@endsection
...
@section('main')
    <header class="md:flex md:justify-center background background-dos w-full">
        <section class="form-container mx-auto w-10/12 md:w-6/12 lg:w-5/12 background background-dos px-6">
            <form id="categoria-crear" class="px-4 my-12 rounded-lg lg:px-4 rounded w-full bg-white none" action="/categoria/crear" method="post"
                data-rules="{{json_encode($validation->rules)}}"
                data-messages="{{json_encode($validation->messages)}}">
                <header>
                    <h2 class="text-2xl text-center py-8">Crear Categoría</h2>
                </header>
                @csrf
                ...
                <div class="mb-4 flex justify-center flex-wrap mb-8 outline-none">
                    <select class="form-input p-3 outline-none bg-white" name="id_tipo_gestion" title="Campo obligatorio">
                        <option {{ !old("id_tipo_gestion") ? 'selected' : '' }} disabled>Tipo de gestión *</option>
                        @foreach($tipos as $tipo)
                            <option value="{{$tipo->id_tipo}}" {{ old("id_tipo_gestion") == $tipo->id_tipo ? 'selected' : '' }}>{{$tipo->nombre}}</option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center justify-center lg:justify-center py-4">
                    <input class="form-submit categoria-crear btn btn-dos p-2 px-8 my-4" type="submit" value="Crear Categoría">
                </div>
            </form>
        </section>
    </header>
...
@section('js')
    ...
    <script type="module" src="{{asset('js/categoria/crear.js')}}"></script>
@endsection
