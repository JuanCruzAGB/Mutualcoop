...
@section('main')
    <header class="md:flex md:justify-center background background-dos w-full">
        <section class="form-container mx-auto w-10/12 md:w-6/12 lg:w-5/12 background background-dos px-6">
            <form id="evento-editar" class="px-4 my-12 rounded-lg lg:px-4 rounded w-full bg-white none" action="/evento/{{$evento->id_evento}}/editar" method="post" enctype="multipart/form-data"
                data-rules="{{json_encode($validation->rules)}}"
                data-messages="{{json_encode($validation->messages)}}">
                <header>
                    <h2 class="text-2xl text-center py-8">Editar "{{$evento->titulo}}"</h2>
                </header>
                @csrf
                @method('PUT')
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="titulo"
                        ...
                        type="text" placeholder="Título *" title="Campo obligatorio" value="{{old('titulo', $evento->titulo)}}">
                ...
                <div class="mb-4 flex justify-center flex-wrap my-8 outline-none">
                    <textarea class="form-input ckeditor descripcion resize-none" id="descripcion" name="descripcion" placeholder="Descripción"
                        cols="100" rows="10">{!!old('descripcion', $evento->descripcion)!!}</textarea>
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    @if(Storage::mimeType($evento->archivo) == 'image/jpeg' || Storage::mimeType($evento->archivo) == 'image/png')
                        <input class="form-input make-a-file make-an-image w-75"
                            ...
                            name="archivo"
                            data-text="Archivo"
                            data-src='{{asset("/storage/$evento->archivo")}}'
                            data-notfound="{{$evento->archivo}}">
                    @else
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input id="calendario" type='text' name="fecha" class='form-input datepicker-here w-full appearance-none border-none p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline'  data-language='es' data-position="bottom left" placeholder="Fecha *" title="Campo obligatorio" value="{{old('fecha', $evento->fecha)}}"/>
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="organizador"
                        class="form-input w-full appearance-none border-none p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        type="text" placeholder="Organizador" value="{{old('organizador', $evento->organizador)}}">
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="video"
                        class="form-input w-full appearance-none border-none p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        type="url" placeholder="Url del video" value="{{old('video', $evento->video)}}">
                    @if($errors->has('video'))
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="url_inscripcion"
                        class="form-input w-full appearance-none border-none p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        type="url" placeholder="Url de la inscripción" value="{{old('url_inscripcion', $evento->url_inscripcion)}}">
                ...
                <div class="checkbox-obras w-8/12 mx-auto xl:w-6/12">
                    <ul class="obras my-8">
                        <li class="checkbox mx-auto w-8/12 md:w-7/12 lg:w-10/12">
                            <input name="privado" value="1" name="privado" {{ (old('privado', $evento->privado) && old('privado', $evento->privado) == 1) ? 'checked' : '' }} class="form-input filter filter-checkbox" id="privado" type="checkbox">
                ...
                <div class="flex items-center justify-center lg:justify-center py-4">
                    <input class="form-submit evento-editar btn btn-dos p-2 px-8 my-4" type="submit" value="Editar Evento">
                </div>
            </form>
        </section>
    </header>
...
@section('js')
    <script>
        const suscriptions = [];
        @if(old('fecha', $evento->fecha))
        const fecha = @json(old('fecha', $evento->fecha));
        @else
        const fecha = '';
        @endif
    ...
    <script type="module" src={{ asset('js/evento/editar.js') }}></script>
@endsection
