...
@section('css')
    ...
    <link href={{ asset('css/gestion/listado.css') }} rel="stylesheet">
@endsection
...
@section('main')
	<div id="tab" class="w-full relative tab-menu">
		...
		<section class="tab-content-list w-full w-full flex justify-center xl:mx-auto">
            @component('components.gestion.lista', [
                'gestiones' => $gestiones,
                'current' => $current,
            ])
            @endcomponent
...
@section('js')
    <script>
        const gestiones = @json($gestiones);
        const suscriptions = [];
    </script>
    <script type="module" src="{{ asset('js/gestion/listado.js') }}"></script>
@endsection