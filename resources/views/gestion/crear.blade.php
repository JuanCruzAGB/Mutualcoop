...
@section('css')
    ...
    <link href={{ asset('css/gestion/crear.css') }} rel="stylesheet">
@endsection
...
@section('main')
    <header class="md:flex md:justify-center background background-dos w-full">
        <section class="form-container mx-auto w-10/12 md:w-6/12 lg:w-5/12 background background-dos px-6">
            <form id="gestion-crear" class="px-4 my-12 rounded-lg lg:px-4 rounded w-full bg-white none" action="/gestion/crear" method="post" enctype="multipart/form-data"
                data-rules="{{json_encode($validation->rules)}}"
                data-messages="{{json_encode($validation->messages)}}">
                <header>
                    <h2 class="text-2xl text-center py-8">Crear Gestión</h2>
                </header>
                @csrf
                ...
                <div class="mb-4 flex justify-center flex-wrap mb-8 outline-none">
                    <select class="form-input p-3 outline-none bg-white" name="id_categoria">
                        <option {{ !old("id_categoria") ? 'selected' : '' }} disabled>Categoría: Seleccione alguna Gestión y una Obra primero</option>
                    </select>
...
@section('js')
    <script>
        ...
        const oldTipo = @json(old('id_tipo_gestion'));
        const categorias = @json($categorias);
        const oldCategoria = @json(old('id_categoria'));
        const oldObras = @json(old('obras'));
        const validation = @json($validation);
    </script>
    <script src={{asset('submodules/InputFileMakerJS/js/InputFileMaker.js')}}></script>
    <script src={{ asset('/vendors/ckeditor/ckeditor.js') }}></script>
    <script type="module" src={{ asset('js/gestion/crear.js') }}></script>
@endsection
