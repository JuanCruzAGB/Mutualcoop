...
$(document).ready(function(){
    ...
    if(alta){
        $('#alta.datepicker-here').datepicker("setDate", new Date(new Date(alta)));
    }
    if(baja){
        $('#baja.datepicker-here').datepicker("setDate", new Date(new Date(baja)));
    }
});
...
document.addEventListener('DOMContentLoaded', function(e){
    ...
    for(const btn of see){
        btn.addEventListener('click', function(e){
            ...
            switch(this.nextElementSibling.type){
                case 'password':
                    input.type = 'text';
