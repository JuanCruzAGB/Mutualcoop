...
@section('css')
    ...
    <link href={{ asset('css/pregunta/crear.css') }} rel="stylesheet">
@endsection
...
@section('main')
    <header class="md:flex md:justify-center background background-dos w-full">
        <section class="form-container mx-auto w-10/12 md:w-6/12 lg:w-5/12 background background-dos px-6">
            <form id="pregunta-crear" class="px-4 my-12 rounded-lg lg:px-4 rounded w-full bg-white none" action="/pregunta/crear" method="post" enctype="multipart/form-data"
                data-rules="{{json_encode($validation->rules)}}"
                data-messages="{{json_encode($validation->messages)}}">
                <header>
                    <h2 class="text-2xl text-center py-8">Crear Pregunta y Respuesta</h2>
                </header>
                @csrf
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="pregunta"
                        ...
                        type="text" placeholder="Pregunta *" title="Campo obligatorio" value="{{old('pregunta')}}">
                ...
                <div class="input-group mb-4 lg:flex lg:justify-center lg:flex-wrap">
                    <input name="respuesta"
                        class="form-input w-full appearance-none border-none p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        type="text" placeholder="Respuesta *" title="Campo obligatorio" value="{{old('respuesta')}}">
                ...
                <div class="flex items-center justify-center lg:justify-center py-4">
                    <input class="form-submit pregunta-crear btn btn-dos p-2 px-8 my-4" type="submit" value="Crear Pregunta y Respuesta">
                </div>
            </form>
        </section>
    </header>
...
@section('js')
    ...
    <script type="module" src="{{asset('js/pregunta/crear.js')}}"></script>
@endsection
